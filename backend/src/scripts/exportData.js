...
const DUMMY_CONCEPTS = [
  ...
  { term: "Câu Trần", category: "Lục Thú", short: "Chủ về sự chậm chạp, trì trệ, ruộng đất, bất động sản, sự trói buộc." },
...
function exportData() {
    ...
    fs.writeFileSync(path.join(dataDir, 'hexagrams.json'), JSON.stringify(hexagrams, null, 2));
    fs.writeFileSync(path.join(dataDir, 'lines.json'), JSON.stringify(lines, null, 2));
    fs.writeFileSync(path.join(dataDir, 'concepts.json'), JSON.stringify(concepts, null, 2));

    console.log(`Generated ${hexagrams.length} hexagrams, ${lines.length} lines, and ${concepts.length} concepts.`);
}
